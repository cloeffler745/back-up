...
db_all = pd.read_sql_query("SELECT * FROM FUNGI_DB WHERE GENUSTAXID IS NULL;", db)
...
for index, row in db_all.iterrows():
	...
	if new_entry: # if the value is not empty insert information as a new row into the current database
		db.execute("UPDATE FUNGI_DB SET STRAINTAXID = ?, STRAINNAME = ?, SPECIESTAXID = ?, SPECIESNAME = ?, GENUSTAXID = ?, GENUSNAME = ?, GIVENTAXID = ? WHERE FILENAME = ?;", (new_entry[0], new_entry[1], new_entry[2], new_entry[3], new_entry[4], new_entry[5], new_entry[7], new_entry[6]))
		db.commit()
...
#	  print(entry) # check, when debugging, that everything looks good
	...
	db.execute("UPDATE FUNGI_DB SET STRAINTAXID=?, STRAINNAME=?, SPECIESTAXID=?, SPECIESNAME=?, GENUSTAXID=?, GENUSNAME=? WHERE FILENAME=?;", (entry[0], entry[1], entry[2], entry[3], entry[4], entry[5], entry[6]))
	db.commit()
