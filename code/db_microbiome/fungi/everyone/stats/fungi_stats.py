...
# Run in terminal to compile list of filepaths
# ls -d "$PWD"/* >> /u/home/c/cloeffle/code/db_microbiome/fungi/everyone/stats/filepaths.list

# BOILER PLATE ---------------------------------------------------------------------------------------------------------

# import sys.path.append(/u/home/c/cloeffle/code/db_microbiome/bacteria/everyone/stats/stats_helper_functions.py)
import stats_helper_functions_corrected as helper
import sqlite3
import re
import gzip
import csv
import pandas as pd
import sys
conn = sqlite3.connect('/u/home/c/cloeffle/scratch/sql/fungi.db')
	...
	with gzip.open(filePath.strip(), 'rb') as f:
		for line in f:
			...
			if line[0] == ">":#only look for assembly level patterns in headers
				dnaCategories = re.findall(r"mitochondri|mt dna|plasmid|chr|\bcomplete\b|NC_|\bpartial\b|contig|scaffold|supercont|unitig_|NW_|NT_", line, re.I) 
