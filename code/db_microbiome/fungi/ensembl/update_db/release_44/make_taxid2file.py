...
ensembl_db = pd.read_sql_query("SELECT * FROM TEST WHERE DBNAME='ENSEMBLE';", db)
...
file_tax = match_taxid_filename(files, taxid_constructed_filenames)

# find matched taxid entries by filename
# delete database entries that are not in the updates
# make matched and updates lists

matched = []# will hold [giventaxid, filepath]

i = 0
for index, row in ensembl_db.iterrows():
	dbfilename = path_strip(row['FILENAME'])
	#print(dbfilename)
	if not dbfilename in file_tax.keys():
		db.execute("DELETE FROM TEST WHERE FILENAME=?;", (row['FILENAME'],))
		#db.commit()
		i += 1
	else: # there is a match
		matched.append([file_tax[dbfilename], row['FILENAME']])
		del file_tax[dbfilename]
print("deleted " + str(i))
# now the file_tax contains things found only in the update 
# now the only entries remaining for ENSEMBL in the table are those that are matched with updates
