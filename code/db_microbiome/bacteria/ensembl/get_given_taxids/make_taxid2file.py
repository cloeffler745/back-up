...
def make_ensmebl_taxid_table(filename):
	...
	for l in h:
		...
		if not [line[3], str(string.capwords(line[1], " ") + "." + line[4].replace(" ", "_") + ".dna.toplevel.fa.gz")] in work_space and not line[0][0] == "#" and line[2] == "EnsemblBacteria":
...
def match_taxid_filename(filename, work_space):
	...
	for b in work_space:
		taxid = b[0]
		first = b[1].split(".")[0]
		file = [s for s in remaining if first in s]
		matched += [[taxid, file[0]]]
...
# put the new table into a text file called "taxID_filename.txt"
