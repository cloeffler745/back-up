...
# ls -d "$PWD"/* >> /u/home/c/cloeffle/code/db_microbiome/bacteria/everyone/stats/filepaths.list
...
import pandas as pd
import sys
conn = sqlite3.connect('/u/home/c/cloeffle/scratch/sql/virus.db')
	...
	with gzip.open(filePath.strip(), 'rb') as f:
		for line in f:
			...
			if line[0] == ">":#only look for assembly level patterns in headers
				dnaCategories = re.findall(r"plasmid|contig|scaffold|chromosome|\bcomplete\b|NC_|\bpartial\b|ctg[0-9]|ctg_[0-9]|supercont|scfld|scf_[0-9]|scf[0-9]|unitig_[0-9]|cont[0-9]", line, re.I)
